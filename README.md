...
- **ログインなしのブラウザ単位のデータ分離**
  ブラウザの `localStorage` に匿名 ID を保持し、その ID でデータを分離。ログイン不要で、
  各ブラウザ（＝各利用者）が自分だけのタスクを持つ
...
| PUT | `/api/categories/reorder` | カテゴリの並び替え |
| DELETE | `/api/categories/:id` | カテゴリ削除 |

> `/api/tasks` と `/api/categories` の各リクエストには、ブラウザが発行する匿名 ID を
> HTTP ヘッダ `X-Client-Id` で付与する必要があります（フロントが自動で付けます）。
> サーバはこの ID でデータを分離し、ヘッダが無いリクエストは 400 を返します。
